...
model = pickle.load(open("student_grade_pred_model2.pkl", "rb"))
scaler = MinMaxScaler()
...
def predict():
    ...
    features = ['Medu', 'Fedu', 'traveltime', 'studytime', 'failures', 'famrel', 'freetime', 'goout', 'health', 'absences', 'G1', 'G2', 'school_GP', 'school_MS', 'sex_F', 'sex_M', 'address_R', 'address_U', 'paid_no', 'paid_yes', 'activities_no', 'activities_yes', 'higher_no', 'higher_yes', 'internet_no', 'internet_yes']
    ...
    'goout': request.form['goout'],
    'health': request.form['health'],
    'absences': float(request.form['absences'])/75,
    'G1': (float(request.form['G1'])-3)/16, #min-max scaling
    'G2': float(request.form['G2'])/19,
    # 'creativity': request.form['creativity'],
    'school_GP': school_GP,
    'school_MS': school_MS,
    'sex_F': gender_F,
    ...
    print(form_data_array)
    # Assuming data is your input data and features is a list of feature names
    data_df = pd.DataFrame(form_data_array, columns=features)
    ...
    return render_template("index1.html", prediction_text = "Predicted Grade of the student is: {}".format(output))
